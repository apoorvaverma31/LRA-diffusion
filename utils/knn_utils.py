...
def knn(query, data, k=10):
    ...
    M = torch.cdist(query, data) # computes the l2 distance between each of the (query,data) pairs. size will be query x data
    # for each sample in the batch, M contains the l2 dist of that sample to all the rest samples in the training dataset
    # M = 1 - torch.mm(query, data.t())
    v, ind = M.topk(k, largest=False) # Returns the k smallest elements of the given input tensor
    # 200 samples, so 200 rows in the output of M.topk
    # v contains the value of the distance, and ind contains the data index (i.e. which particular sample is closer to the sample in the batch)
...
def sample_knn_labels(query_embd, y_query, prior_embd, labels, k=10, n_class=10, weighted=False):
    ...
    _, neighbour_ind = knn(query_embd, prior_embd, k=k) # for each query embedding, returns the indexes of the k-nearest neighbours 
    # in the latent space generated by the simclr encoder
    ...
    # now this contains 11 dimensions, 10 for the label of neighbours and 1 for its own label
    ...
    # this is the neighbourhood consistency step wherein for each datapoint in the batch, we are randomly choosing a label for it
    # from the labels of its k nearest neighbours.
    ...
    # torch.sum(y_one_hot_batch, dim=1): gives the classwise number of neighbours for each datapoint in the batch
    # attaching the [...] thing returns the number of times the sampled label (derived above) appears in the neighbourhood of 
    # each datapoint in the batch.
